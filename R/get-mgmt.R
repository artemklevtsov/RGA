...
get_mgmt <- function(path, token) {
    ...
    data_list$created <- as.POSIXct(strptime(data_list$created, format = "%Y-%m-%dT%H:%M:%OS", tz = "UTC"))
    data_list$updated <- as.POSIXct(strptime(data_list$updated, format = "%Y-%m-%dT%H:%M:%OS", tz = "UTC"))
    return(data_list)
}
