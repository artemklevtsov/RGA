...
After calling this function first time, a web browser will be opened. If `login` argument was not specified you need choose an account for the authorization. First entrance with a **Google Account** confirms access to the Google Analytics data. Note that the package requests access for the **read-only** data.
...
When the `cache` argument and the `rga.cache` option is not changed and `login` param is not specified, then after successful authorization the `.ga-token.rds` file with access token to Google API will be created in the current working directory. The `.ga-token.rds` file is used between sessions, i.e. at a subsequent call to the `authorize()`  function and authorization in the browser tab is not required. If `login` argument specified token will be cached in the `.username-token.rds` file.
...
Also you can specify `login` argument:

```r
work_token <- authorize(login = "home@domain.com")
home_token <- authorize(login = "work2@domain.com")
```
